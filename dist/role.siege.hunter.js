...
function run(creep) {
    ...
    function doBattle() {
        ...
        var nearbyHostileCreeps = creep.pos.findInRange(hostileCreeps, 3);
        attackSomething();
        pursueSomething();
        ...
        function hostileCreepPriority(hostileCreep) {
            ...
            var numberOfBodyPartsICanKill = Math.min(activeParts.length, (myDamage >= activeParts[0].hits ? 1 : 0) + Math.floor((myDamage - activeParts[0].hits) / 100));
            var numberOfImportantPartsICanKill = util.filter(activeParts.slice(0, numberOfBodyPartsICanKill), o => o.type === ATTACK || o.type === RANGED_ATTACK || o.type === HEAL).length;
            var numberOfImportantPartsTotal = util.filter(hostileCreep.body, o => o.type === ATTACK || o.type === RANGED_ATTACK || o.type === HEAL).length;
...
exports.run = run;
//# sourceMappingURL=role.siege.hunter.js.map