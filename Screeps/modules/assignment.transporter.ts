...
export function assignTransporters() {
    ...
    function assign(target: Structure, priority: number, amountNeeded: number) {
        const transporters: Array<Creep> = [];
        ...
        const transportersAlreadyAssignedToThisTarget = util.filter(transporters, o =>
...
function assignToTransporter(transporter: Creep, target: Structure, priority: number, amountNeeded: number) {
...
function switchModes(roomName) {
    ...
    for (var name in Game.creeps) {
        ...
        if (creep.memory.role !== enums.TRANSPORTER || creep.memory.assignedRoomName !== roomName) continue;
