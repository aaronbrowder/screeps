...
export function run() {
    ...
    function runRoomSpawn(roomName: string, doClaim: boolean) {
        ...
        spawnMapInfos.forEach(o => {
            const spawn: SpawnInfo = _.filter(allSpawns, (p: SpawnInfo) => p.id === o.id)[0];
            spawn.distance = o.distance;
            spawns.push(spawn);
        });
        spawns = _.sortBy(spawns, (o: SpawnInfo) => o.distance / o.maxEnergyAvailable);
        ...
        function areSpawnsUsable() {
            return spawns.length && spawns.some(o => {
                const spawn = Game.getObjectById<StructureSpawn>(o.id);
        ...
        var idealTotalTransporterCarryParts =
        ...
        function spawnForWartime() {
            ...
            for (let i = 0; i < spawns.length; i++) {
                if (doClaim) {
                    if (spawnTransporters(spawns[i])) return;
                    const attackPower = _.sum(getCreeps(roomName, 'meleeMercenary'), (o: Creep) => o.getActiveBodyparts(ATTACK) * 1.5);
                    const rangedAttackPower = _.sum(getCreeps(roomName, 'rangedMercenary'), (o: Creep) => o.getActiveBodyparts(RANGED_ATTACK));
                    if (attackPower < rangedAttackPower) {
                        if (spawnMeleeMercenary(spawns[i])) return;
                    }
                    if (spawnRangedMercenary(spawns[i])) return;
                } else {
        ...
        function spawnForPeacetime() {
            ...
            if (!doClaim) {
                ...
                spawnsToUse = _.filter(allSpawns, (o: SpawnInfo) => o.id === Game.spawns['Spawn2'].id);
            ...
            for (let i = 0; i < spawnsToUse.length; i++) {
                if (spawnBuilders(spawnsToUse[i], 'wallBuilder', idealTotalWallBuilderWorkParts)) return;
                if (spawnBuilders(spawnsToUse[i], 'upgrader', idealTotalUpgraderWorkParts)) return;
                if (spawnHarvesters(spawnsToUse[i])) return;
                if (spawnTransporters(spawnsToUse[i])) return;
            }
            if (doClaim) {
                // spawn a hub
        ...
        function spawnHarvesters(spawnInfo: SpawnInfo) {
            ...
            for (let i = 0; i < activeSources.length; i++) {
                if (spawnHarvestersForAssignment(activeSources[i], idealHarvesterWorkPartsPerSource)) return true;
            }
            for (let i = 0; i < activeMinerals.length; i++) {
                if (spawnHarvestersForAssignment(activeMinerals[i], idealHarvesterWorkPartsPerMineral)) return true;
            }
            return false;

            function spawnHarvestersForAssignment(assignment: Source | Mineral, idealWorkParts: number) {
                var assignedHarvesters = _.filter(harvesters, o => o.memory.assignmentId === assignment.id);
                var workParts = _.sum(assignedHarvesters, (o: Creep) => o.getActiveBodyparts(WORK));
            ...
            function spawnHarvester(assignedHarvesters: Creep[], assignment: Source | Mineral, workParts: number, idealTotalWorkParts: number) {
        ...
        function spawnTransporters(spawnInfo: SpawnInfo) {
            ...
            var totalCarryParts = _.sum(transporters, (o: Creep) => o.getActiveBodyparts(CARRY));
            ...
            function spawnTransporter(carryParts: number) {
        ...
        function spawnBuilders(spawnInfo: SpawnInfo, subRole: string, idealTotalWorkParts: number) {
            var builders = getCreeps(roomName, 'builder', subRole);
            var totalWorkParts = _.sum(builders, (o: Creep) => o.getActiveBodyparts(WORK));
            ...
            function spawnBuilder(workParts: number) {
        ...
        function spawnMeleeMercenary(spawnInfo: SpawnInfo) {
        ...
        function spawnRangedMercenary(spawnInfo: SpawnInfo) {
        ...
        function spawnRavager(spawnInfo: SpawnInfo) {
            const role = 'ravager';
            const body = [
                TOUGH, TOUGH, TOUGH, TOUGH, TOUGH,
        ...
        function spawnColonists(spawnInfo: SpawnInfo, count: number, claimParts: number) {
        ...
        function spawnClaimers(spawnInfo: SpawnInfo) {
            const currentClaimerParts: number = _.sum(getCreeps(roomName, 'claimer').map(o => o.getActiveBodyparts(CLAIM)));
    ...
    function getCreeps(roomName: string, role: string, subRole?: string) {
        var creeps: Creep[] = [];
    ...
    function sortCreeps(creeps: Creep[]) {
