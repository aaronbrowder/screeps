...
export function assignBuilders() {
    ...
    for (let roomName in Game.rooms) {
        ...
        for (let i in structures) {
            ...
            else if (target.structureType == STRUCTURE_CONTROLLER && ((target as StructureController).level === 1 || isCriticallyDowngrading(target))) {
                assign(target, 3);
            }
        ...
        for (let i in constructionSites) {
            ...
            if (target.structureType == STRUCTURE_WALL || target.structureType == STRUCTURE_RAMPART) {
                ...
                if (isMyRoom) assign(target, 10);
                else assign(target, 5);
            }
