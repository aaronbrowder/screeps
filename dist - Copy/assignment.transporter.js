...
function assignTransporters() {
    ...
    for (let roomName in Game.rooms) {
        ...
        for (let i = 0; i < structures.length; i++) {
            ...
            if (target.structureType == STRUCTURE_CONTAINER && _.sum(target.store) > target.storeCapacity * 0.25
            && target.pos.findInRange(FIND_MINERALS, 2).length) {
                assign(target, 4, _.sum(target.store));
            }
            if (target.structureType == STRUCTURE_CONTAINER && _.sum(target.store) < target.storeCapacity * 0.85
...
module.exports = {
    assignTransporters: assignTransporters
};